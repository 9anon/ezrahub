    ...
    $(document).on('keyup', 'textarea.enhanced', function () {
        ...
        $.each(wordcount, function (k, v) {
            ...
            if (finalcount >= cutoff.fourth) {
                update_rating('four', content.fifth, colors.fifth, '100%');
            } else if (finalcount >= cutoff.third) {
                update_rating('three', content.fourth, colors.fourth, '80%');
            } else if (finalcount >= cutoff.second) {
                update_rating('two', content.third, colors.third, '60%');
            } else if (finalcount >= cutoff.first) {
                update_rating('one', content.second, colors.second, '40%');
            } else if (finalcount != 0) {
                update_rating('zero', content.first, colors.first, '15%');
            } else {
                update_rating('zero', content.first, colors.first, '0%');
            }
        });
        ...
        //create an array of all the words in the textarea
        if (typeof $('textarea.enhanced').val() != 'undefined') {
            words = $('textarea.enhanced').val().split(/\b/g);
        }

        //filter array to only include elements that are filtered
        var caught = words.filter(is_filtered_word);
        if (caught.length > 0) {
            update_rating('filteredword', content.filtered, colors.filtered, '100%');
            var wordsstring = '';
            var filteredwords = '';
            if (caught.getUnique().length > 2) {
                wordsstring = 'different words';
                filteredwords = caught.getUnique().join(', ');
            } else if (caught.getUnique().length > 1) {
                wordsstring = 'different words';
                filteredwords = caught.getUnique().join(' and ');
            } else {
                wordsstring = 'a different word'
                filteredwords = caught.getUnique();
            }
            $('div.progressbar').html('Please try to use ' + wordsstring +  ' than <span class="filtered-word">' + filteredwords + '</span>!');
        } else {
            //clear the progress bar text if there are no filtered words anymore
...
function update_rating(spanclass, content, color, progress) {
    ...
    if (spanclass === 'filteredword') {
        $('div.progress-bar').css('background', '#333333');
    } else {
        $('div.progress-bar').css('background', color);
    }
    if (typeof progress != 'undefined') {
        $('div.progress-bar').stop().animate({
            width: progress
        }, 125);
    }
}

//function to check if a word is in the filtered array
function is_filtered_word(element, index, array) {
    //check if element is in the filtered words array (returns -1 if not in array)
